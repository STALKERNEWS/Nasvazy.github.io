...
$album = $database->database_query("SELECT * FROM se_video_albums WHERE album_id='$album_id' AND album_user_id='".$user->user_info[user_id]."'");
if($database->database_num_rows($album) != 1) { header("Location: user_video.php"); exit(); }
...
if($task == "dosave") {
  ...
  if($is_error == 0) {
    ...
    $database->database_query("UPDATE se_video_albums SET album_title='$album_title',
    ...
    $result = 1;
    $album_info = $database->database_fetch_assoc($database->database_query("SELECT * FROM se_video_albums WHERE album_id='$album_id' AND album_user_id='".$user->user_info[user_id]."'"));
  }
}
