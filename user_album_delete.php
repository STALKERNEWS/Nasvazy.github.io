...
if($database->database_num_rows($album) != 1) { header("Location: user_album.php?user_id='".$user->user_info[user_id]."'"); exit(); }
...
if($task == "dodelete") {
  ...
  header("Location: user_album.php?user_id=".$user->user_info[user_id]."");
  exit();
}
