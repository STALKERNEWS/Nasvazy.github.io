...
if( $task == "upload" )
{
  ...
  if( !$is_error )
  {
    ...
    $actions->actions_add($user, "editphoto", Array($user->user_info['user_username'], $user->user_displayname), $action_media, 999999999, TRUE, "user", $user->user_info['user_id'], $user->user_info['user_privacy']);
                mysql_query("INSERT INTO se_profilecomments (profilecomment_user_id, profilecomment_authoruser_id, profilecomment_obn) VALUES ('".$user->user_info[user_id]."', '".$user->user_info[user_id]."', '1')");

 header("Location: ./id".$user->user_info[user_id]."");
 }
}
