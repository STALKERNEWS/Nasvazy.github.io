...
{
echo '4658<!><!>0<!>4250<!>0<!>Пользователь был удален из списка Ваших друзей.';
  $user->user_friend_remove($owner->user_info['user_id']);
  $status = "add";
  $result = 890;
}
...
echo '4658<!><!>0<!>4250<!>0<!><b>Заявка отправлена.</b><br /><a class="mem_link" href="/id'.$owner->user_info['user_id'].'" onclick="return nav.go(this, event)">'.$owner->user_displayname.'</a> получил уведомление и должен будет подтвердить, что Вы его друг.';
}  
