...
if($task == "block") {
  ...
  if($user->user_blocked($owner->user_info[user_id])) { $result = $user_fans_block[3]; $confirm = 0; $is_error = 1; }

  // CHECK IF USER IS TRYING TO BLOCK THEMSELVES
  if($owner->user_info[user_id] == $user->user_info[user_id]) { $result = $user_fans_block[2]; $confirm = 0; $is_error = 1; }

  // DISPLAY ERROR PAGE IF NO OWNER
  if($owner->user_exists == 0) { $result = $user_fans_block[1]; $confirm = 0; $is_error = 1; }


  // DISPLAY ERROR IF NECESSARY
  if($is_error == 1) {
    $page = "error";
    $smarty->assign('error_header', $user_fans_block[4]);
    $smarty->assign('error_message', $result);
    $smarty->assign('error_submit', $user_fans_block[21]);
    include "footer.php";
  }
  ...
  if($confirm == 1) {
    ...
    $user->user_fan_remove($owner->user_info[user_id]);

    $result = $user_fans_block[5]." ".$owner->user_info[user_username].".";
    $confirm = 0;
...
if($task == "unblock") {
  ...
  if($user->user_blocked($owner->user_info[user_id]) == FALSE) { $result = $user_fans_block[13]; $confirm = 0; $is_error = 1; }

  // CHECK IF USER IS TRYING TO UNBLOCK THEMSELVES
  if($owner->user_info[user_id] == $user->user_info[user_id]) { $result = $user_fans_block[14]; $confirm = 0; $is_error = 1; }

  // DISPLAY ERROR PAGE IF NO OWNER
  if($owner->user_exists == 0) { $result = $user_fans_block[15]; $confirm = 0; $is_error = 1; }
  ...
  if($confirm == 1) {
    ...
    $result = $user_fans_block[16]." ".$owner->user_info[user_username].".";
    $confirm = 0;
